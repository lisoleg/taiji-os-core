...
2. **已完成（v4.9.0）**：FLUX 语义放宽至 100% 覆盖（11/11 轮全 FLUX）+ TruthfulQA 扩展到 817 题完整数据集（50 题快速验证中）
3. **v5.0 规划中**：TruthfulQA 完整 817 题评测运行；SWE-bench + GAIA 外部基准；decay 超参自动调优（基于主题切换速率自适应调整 γ）
4. **长期**：与 vLLM/Strata 集成，在真实 LLM 推理引擎中验证 USCS 抽象；δ-mem S 矩阵生命周期的内核级管理
