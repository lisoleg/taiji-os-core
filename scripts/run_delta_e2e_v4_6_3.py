"""
run_delta_e2e_v4_6_3.py — δ-mem v4.6.3 降 β 端到端验证

v4.6.3 核心改动（相对 v4.6.2）:
  - D-Core S ingest 漂移时不跳过，改为 β ← β × 0.2（阻尼 80%）
  - 预期：FLUX_ENABLED ≥ v4.6.2 (36.4%)，漂移期间 ΔS 非零但被阻尼
...
对比基线:
  ...
  - v4.6.3 降 β:  目标 FLUX_ENABLED ≥ 40%

环境: sentence-transformers + DeepSeek Chat API
用法: python scripts/run_delta_e2e_v4_6_3.py [--rounds N] [--quick]
"""
...
logger = logging.getLogger("delta_e2e_v463")

# ==========================================================================
# E2E 测试场景 — 与 v4.5.0/v4.6.0 完全相同，保证可比性
# ==========================================================================
...
def run_e2e(rounds_per_stage: Optional[int] = None):
    ...
    """
    print("=" * 72)
    print("δ-mem E2E Validation v4.6.3 — β-Reduced Drift Strategy")
    print("=" * 72)
    ...
    print(f"    v4.6.3 Feature: D-Core β→{original_beta*0.2} during drift (reduced, NOT paused)")
    ...
    for i, env in enumerate(all_rounds):
        ...
        # v4.6.3: 漂移期间 ΔS 非零是期望行为（β降低但未暂停）
        if drift_post and S_delta != 0:
            drift_tag = "⏸ β↓" if S_delta < 1e-6 else "⏸ β↓"
        ...
        print(f"  [{i+1:2d}/{total}] {stage:7s} {status} | {flux_tag:5s} | "
              ...
              f"ΔS={f'{S_delta:+.8f}' if S_delta != 0 else '0'} | "
    ...
    # ── 汇总统计 ──
    print("\n" + "=" * 72)
    print(f"结果汇总 — v4.6.3 β-Reduced Drift")
    print("=" * 72)
    ...
    if drift_rounds_num:
        ...
        print(f"    首次触发: 第 {first_drift} 轮 (v4.6.0=第2轮, v4.6.2=第5轮)")
    ...
    # ── 历史基线对比 ──
    print(f"\n  📊 历史基线对比:")
    print(f"    {'指标':<30} {'v4.5.0':>10} {'v4.6.0':>10} {'v4.6.2':>10} {'v4.6.3':>10}")
    print(f"    {'─'*30} {'─'*10} {'─'*10} {'─'*10} {'─'*10}")
    print(f"    {'FLUX_ENABLED':<30} {'27.3%':>10} {'18.2%':>10} {'36.4%':>10} {f'{flux_ratio*100:.1f}%':>10}")
    print(f"    {'通过率':<30} {'27.3%':>10} {'100%':>10} {'100%':>10} {f'{accepted/total*100:.0f}%':>10}")
    print(f"    {'Φ 均值':<30} {'-0.11':>10} {'+0.35':>10} {'+0.34':>10} {f'{np.mean(phi_vals):+.2f}':>10}")
    print(f"    {'漂移首触发':<30} {'-':>10} {'第2轮':>10} {'第5轮':>10} {f'第{drift_rounds_num[0]+1 if drift_rounds_num else "?"}轮':>10}")
    print(f"    {'D-Core策略':<30} {'暂停':>10} {'跳过':>10} {'跳过':>10} {'降β':>10}")
    ...
    out_path = out_dir / "delta_e2e_v4_6_3.json"
    ...
    # v4.6.3: 漂移期间 ΔS 应为非零（β 降级允许学习）
    if drift_indices := [i for i, f in enumerate(drift_flags) if f]:
        drift_deltas = [abs(results[i]["S_delta"]) for i in drift_indices
                       if results[i]["S_delta"] != 0]
        non_drift_deltas = [abs(results[i]["S_delta"]) for i in range(total)
                           if not drift_flags[i] and results[i]["S_delta"] != 0]

        if drift_deltas:
            avg_drift_delta = np.mean(drift_deltas)
            print(f"    v4.6.3: 漂移期间平均 |ΔS| = {avg_drift_delta:.8f} (β↓ 阻尼后)")
        if non_drift_deltas:
            avg_norm_delta = np.mean(non_drift_deltas)
            print(f"    v4.6.3: 稳态期间平均 |ΔS| = {avg_norm_delta:.8f}")
            if drift_deltas:
                ratio = avg_drift_delta / avg_norm_delta if avg_norm_delta > 0 else 1.0
                print(f"    v4.6.3: 漂移/稳态 ΔS 比 = {ratio:.3f} (期望 ~0.2, 对应 β×0.2)")

        for idx in drift_indices:
            delta = results[idx]['S_delta']
            is_zero = delta == 0
            print(f"      轮次 {idx+1}: ΔS={delta:+.8f} "
                  f"{'✓ β-DAMPED (expected)' if not is_zero else '⚠ ZERO (unexpected in v4.6.3)' if is_zero else ''}")
    else:
        print(f"    无漂移触发 — S 持续学习")
    ...
    print(f"      RECOVERY:{recover_flux}/{total-stable_count-drift_count} "
          f"({recover_flux/(total-stable_count-drift_count)*100:.0f}%)")

    print("\n" + "=" * 72)
    print(f"v4.6.3 β-Reduced Drift E2E 验证完成！")
    print("=" * 72)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="δ-mem v4.6.3 β-Reduced Drift E2E Validation")
